    ...
    for(var lineItemFieldName in lineItemHash) {
        for(var index = 0; index < lineItemHash[lineItemFieldName].length; index++) {
            ...
            for(lineItemField in lineItemHash[lineItemFieldName][index]) {
                record.setLineItemValue(lineItemFieldName, lineItemField, index+1,
...
function populateAddressBook(record, sublist, lineItemArray) {
  for(var i = 0; i < lineItemArray.length; i++) {
    line = record.findLineItemValue(sublist, 'label', lineItemArray[i]['label']);
    line > 0 ? record.selectLineItem(sublist, line) : record.selectNewLineItem(sublist);
    for(lineItemField in lineItemArray[i]) {
      result = record.setCurrentLineItemValue(sublist, lineItemField, lineItemArray[i][lineItemField]);
    }
...
function populateList(record, sublist, lineItemArray) {
  for(var i = 0; i < lineItemArray.length; i++) {
...
function removeAllLineItems(record, field) {
  var line_count = record.getLineItemCount(field);
  for(var i = 0; i < line_count; i++) {
    record.removeLineItem(field, 1);
  }
  return record
}
    ...
    for(var index = 0; index < recordData.length; index++) {
        ...
        try {
            ...
            for(var field in attributes) {
                if(field=='addressbook') { record = populateAddressBook(record, field, attributes[field]); }
                else if(field=='item' || field=='creditcards' || field=='giftcertredemption') {
                  if(field=='item') { record = removeAllLineItems(record, field); }
                  record = populateList(record, field, attributes[field]); }
                else if(field=='price') {
                  record.selectLineItem('price', 5);
                  record.setCurrentLineItemMatrixValue('price', 'price', 1, attributes[field]);
                  record.commitLineItem('price');}
                else if(field=='reorderpoint') { record.setLineItemValue('locations','reorderpoint', 13, attributes[field]); }
                else { record.setFieldValue(field, attributes[field]); }
